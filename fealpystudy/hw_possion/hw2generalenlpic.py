...
c = 1+x**2+y**2
#c = np.array([1000.0],dtype=np.float64)
...
for i in range(4):
    ...
    if len(As.shape) == 2:
        A = np.einsum('i, ijkl, ln, ijmn, j->jkm', ws, gphi, As, gphi, cellmeasure) #(NC, ldof, ldof)
    else:
        A = np.einsum('i, ijkl, ijln, ijmn, j->jkm', ws, gphi, As, gphi, cellmeasure)
